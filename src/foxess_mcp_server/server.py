...
SERVER_INSTRUCTIONS = (
    ...
    "as an approximation. The same exact counter-difference method also yields "
    "`feedin_today_kwh`, `grid_consumption_today_kwh`, `charge_today_kwh` and "
    "`discharge_today_kwh` (all in report_day + realtime, grouped under `exact_daily`); "
    "prefer these over the plain report `feedin`/`gridConsumption`/charge/discharge "
    "totals, which are rounded to 0.1 kWh and lag on the current day. "
    "report_day/report_month's raw fields are only: generation, feedin, "
...
_ANALYSIS_GENERATION_NOTE = (
    ...
    "generation + charge_energy_total. The same counter-difference method also yields "
    "exact `feedin_today_kwh`, `grid_consumption_today_kwh`, `charge_today_kwh` and "
    "`discharge_today_kwh` (under `exact_daily`) — prefer these over the rounded/lagged "
    "raw report totals. report_day/report_month's raw fields are ONLY: generation, "
